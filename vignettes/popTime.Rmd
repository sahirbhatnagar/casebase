...
Note that the [`theme.params`](https://ggplot2.tidyverse.org/reference/theme.html) argument is a list of arguments passed to `ggplot2::theme()` function. 
...
The user can have greater control over the aesthetics of the plot by specifying these in the `.params` arguments. For example, suppose we want to change the color of the points and the legend labels. We can do so by overriding . We illustrate the inner workings of the `plot` method for objects of class `popTime` and `popTimeExposure` so that the user may further customize the plot aesthetics if they're not satisfied with the defaults. We use the `bmtcrr` dataset as the example in this section. 
...
plot(popTimeData,
     ...
     legend.params = list(name = "Legend",

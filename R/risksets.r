...
DT[,`:=` ("t1"=rweibull(nobs, a1, b1 * exp(z * c1)^(-1/a1)),
          "t2"=rweibull(nobs, a2, b2 * exp(z * c2)^(-1/a2)),
          "tlim"=10)]
DT[,`:=`("e"=1 * (t1 < t2) + 2 * (t1 >= t2), "t"=pmin(t1,t2))]
DT[t >= tlim, e:=0]
DT[t >= tlim, t:=tlim]
DT[, idx:=order(t)]
...
for (i in 1:nobs) {
    if (e[i] == 1) {
        # yc: for a given subject i with event=1, sample the id numbers from subjects who have
        # an event time (either from being censored or event type=2) greater than subject i 
...
with(DT,segments(rep(0.0, length(z)), ycoord, t, ycoord, col='gray'))
with(DT[z==1],segments(rep(0.0, length(z)), ycoord, t, ycoord, col='yellow'))
with(DT[z==],points(t[e==1], ycoord[e==1], pch=20, col='blue', cex=0.5))
par(op)

...
#'
#' @param fit Output of function \link{\code{fitSmoothHazard}}
...
#'   which simply calls the function \link{\code{integrate}}. The only other option is \code{"montecarlo"}, which implements Monte-Carlo integration.
#' @param nsamp Maximal number of subdivisions (if \code{method = "quadrature"}) or number of sampled points (if \code{method = "montecarlo}).
#' @return Returns the mean absolute risk at the user-supplied time.
absoluteRisk <- function(fit, time, method = c("quadrature", "montecarlo"), nsamp=100) {
    ...
    if (method == "quadrature") {
        for (i in 1:nobs) {
            surv[i] <- exp(-integrate(lambda, lower=0, upper=time, pred=modelMat[i,], beta=coefficients(fit),
    ...
    if (method == "montecarlo") {
        ...
        for (i in 1:nobs) {
            surv[i] <- exp(-mean(lambda(sampledPoints, pred=modelMat[i,], beta=coefficients(fit))))
        }
    }

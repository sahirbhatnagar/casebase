...
#'   (0) [numeric]} \item{Follow.Up.Time}{The time, measured in years from
#'   randomization, at which follow-up was terminated} \item{DeadOfPrCa}{Whether
#'   follow-up was terminated by Death from Prostate Cancer (1) or by death from
#'   other causes, or administratively (0)} }
#'
...
#'   the Postcript code that the NEJM article (Schröder et al.) used to render
#'   Figure 2 (see Liu et al. for details). The uncensored values were more
#'   difficult to recover exactly, as the ‘jumps’ in the Nelson-Aalen plot are
#'   not as monotonic as first principles would imply. Thus, for each arm, the
#'   numbers of deaths in each 1-year time-bin were estimated from the

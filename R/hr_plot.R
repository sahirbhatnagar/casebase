#' New function to plot with casebase objects
#'
#' @description THIS IS A WORK IN PROGRESS. Visualize estimated hazard curves as
#'   a funtion of time with confidence intervals. This function takes as input,
#'   the result from the [casebase::fitSmoothHazard()] function. The user can
#'   also specify a sequence of times at which to estimate the hazard function.
#'   These plots are useful to visualize the non-proportional hazards, i.e.,
#'   time dependent interactions with a covariate.
...
#' @export
hazardPlotNew <- function(object, newdata, type = c("hazard","hr"), xlab = NULL,
    ...
    # for "glm", if ci = F this returns a vector, if ci=T it returns a list
    # of length 3 --> list(fit, se.fit, residual.scale)
    preds <- estimate_hazard(object, newdata = newdata, ci = ci, type = "link")
    newdata$predictedloghazard <- if (ci) preds[["fit"]] else preds
    # newdata$predictedloghazard <- preds[["fit"]]
